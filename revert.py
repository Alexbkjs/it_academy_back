    ...
    async with AsyncSessionLocal() as session:
        async with session.begin():
            ...
            if achievements:
                ...
                formatted_ids = ",".join(
                    f"'{achievement_id}'" for achievement_id in achievement_ids
                )
    ...
    async with AsyncSessionLocal() as session:
        async with session.begin():
            ...
            if requirements:
                ...
                formatted_ids = ",".join(
                    f"'{requirement_id}'" for requirement_id in requirement_ids
                )
    ...
    async with AsyncSessionLocal() as session:
        async with session.begin():
            ...
            if user_quest_progress:
                ...
                await session.execute(text(query))
                print(
                    f"Deleted {len(user_quest_progress)} user quest progress records."
                )
        await session.commit()
